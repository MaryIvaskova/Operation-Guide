...
class App(models.Model):
    name = models.CharField(
        max_length=100,
        validators=[RegexValidator(regex=r'^[\w\s\-]+$', message='Назва може містити лише літери, цифри, пробіли та дефіси.')]
    )
    os = models.CharField(
        max_length=20,
...
class Instruction(models.Model):
    ...
    title = models.CharField(
        max_length=255,
        validators=[RegexValidator(regex=r'^.{5,}$', message='Заголовок має бути не коротший 5 символів.')]
    )
...
class Feedback(models.Model):
    text = models.TextField(
        validators=[RegexValidator(regex=r'^.{10,}$', message='Відгук має бути не менше 10 символів.')]
    )
...
class Review(models.Model):
    name = models.CharField(
        max_length=100,
        validators=[RegexValidator(regex=r'^.{2,}$', message='Імʼя має бути не менше 2 символів.')]
    )
    rating = models.IntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(5)]
    )
    comment = models.TextField(
        validators=[RegexValidator(regex=r'^.{10,}$', message='Коментар має бути не менше 10 символів.')]
    )
    created_at = models.DateTimeField(auto_now_add=True)
